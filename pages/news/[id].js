...
export default function Single(props) {
    ...
    useEffect(() => {
        if (props.articolo.meta.pagination.total > 0) {
            ...
            setTitolo(props.title.rendered);
            setContent(props.content.rendered);
            setDataPub(props.date);
            setCopertina('../images/prova2.png');
            setAutore(props.authorName);
            setLoading(true);
        }
        else {
...
export async function getServerSideProps(context) {
    ...
    const articolo = await fetch(process.env.NEXT_PUBLIC_API_URL + "/posts/" + id).then(r => r.json())
    return {
        props: {
            articolo: articolo
        },
